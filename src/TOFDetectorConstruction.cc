 ...
 fCheckOverlaps(true)
{
    ...
    NumberOfS2 = 4;
    DefineMaterials();
    DetectorMessenger = new TOFDetectorMessenger(this);
}
...
G4VPhysicalVolume* TOFDetectorConstruction::Construct()
{
    ...
    G4double expHall_x = 300.0 * cm;
    G4double expHall_y = 300.0 * cm;
    G4double expHall_z = 300.0 * cm;
...
void TOFDetectorConstruction::DefineVolumes()
{
    ...
    */
    ...
    {
        G4double phi = i * 180.0 / NumberOfS1 * deg;
        G4double pos_z =-TOFRadius + (2 * i + 1 - NumberOfS1) * (S1_scinHalfThickness + 0.5 * mm);
    ...
    //S2_scintillator
    G4double S2_scinhy = 8.5 * mm;      // Half thickness  of S2
    G4double S2_scinhx1 = 35.0 * mm;    // Half length of S2 top side in x axis
    G4double S2_scinhx2 = 50.0 * mm;    // Half length of S2 bottom side in x axis
    G4double S2_scinhz  = 140.0 * mm;   // Half length of S2 in z axis
    G4Trd* S2_scinSV = new G4Trd("S2_scinSV", S2_scinhx2, S2_scinhx1, S2_scinhy, S2_scinhy, S2_scinhz);
    S2_scinLV = new G4LogicalVolume(S2_scinSV, S2Material, "S2_scinLV");
    
    //S2 fishtail light guide
    G4Box* S2lg1 = new G4Box("S2lg1", 50 * mm, 8.5 * mm, 30 * mm);
	G4Trd* S2lg2 = new G4Trd("S2lg2", 21.7 * mm, 50 * mm, 21.7 * mm, 8.5 * mm, 35 * mm);
    G4Tubs*	S2lg4 = new G4Tubs("S2lg4", 0.0 , 25 * mm, 25 * mm, 0. * deg, 360 * deg);
	G4UnionSolid* S2lg3 = new G4UnionSolid("s2lg3", S2lg1, S2lg2, 0 , G4ThreeVector(0.0, 0.0, -65*mm)); 
	G4UnionSolid* S2_LGSV = new G4UnionSolid("s2lg", S2lg3, S2lg4, 0, G4ThreeVector(0.0,0.0,-125*mm)); 
	S2_LGLV = new G4LogicalVolume(S2_LGSV, LGMaterial, "S2_LGLV");
    
    //assemble all the parts of S2
    G4AssemblyVolume* S2_detector = new G4AssemblyVolume();
    G4ThreeVector Tau;
    G4Transform3D Tru;
    
    /*
    // fill the S2 with S2_LGLV
    Tau.setZ(-170 * mm);
    G4RotationMatrix Rau_LG;
    Tru = G4Transform3D(Rau_LG, Tau);
    S2_detector->AddPlacedVolume(S2_LGLV, Tru);
    
    // fill the S2 with S2_MSLV
    Tau.setZ(-410 * mm);
    G4RotationMatrix Rau_MS;
    Tru = G4Transform3D(Rau_MS, Tau);
    S2_detector->AddPlacedVolume(S2_MSLV, Tru);
    */
    
    // fill the S2 with S2_sinLV
    Tau.setZ(0 * mm);
    G4RotationMatrix Rau_scin;
    Tru = G4Transform3D(Rau_scin, Tau);
    S2_detector->AddPlacedVolume(S2_scinLV, Tru);
    
    //S2detector placement
    for (G4int i = 0; i < NumberOfS2; i++)
    {
        G4double theta = flightAngle * 2;
        G4double phi = i * 360 / NumberOfS2 * deg;
        ...
        S2_detector->MakeImprint(expHallLV, transform);
    }
    ...
    G4VisAttributes* S2_scinVisAtt = new G4VisAttributes(red);
    S2_scinLV -> SetVisAttributes(S2_scinVisAtt);
    
    G4VisAttributes* S2_LGVisAtt = new G4VisAttributes(blue);
    S2_LGLV -> SetVisAttributes(S2_LGVisAtt);
    
    //---------Output geometry parameters---------
    G4cout << "Flightangle zzz : " << flightAngle / deg << " deg"  << G4endl;
    G4cout << "NumberOfS1 : " << NumberOfS1 << G4endl;
    G4cout << "NumberOfS2 : " << NumberOfS2 << G4endl;
...
void TOFDetectorConstruction::UpdateGeometry()
{
...
void TOFDetectorConstruction::ConstructSDandField()
{
    ...
    TOFscinSD* S1_scinSD = new TOFscinSD(SDname = "S1_scinSD", HCname = "s1_hitscollection");
    SDmanager->AddNewDetector(S1_scinSD);
    S1_scinLV->SetSensitiveDetector(S1_scinSD);
    
    TOFscinSD* S2_scinSD = new TOFscinSD(SDname = "S2_scinSD", HCname = "s2_hitscollection");
    SDmanager->AddNewDetector(S2_scinSD);
    S2_scinLV->SetSensitiveDetector(S2_scinSD);
}
